        ...
        .setup(|app| {
            ...
            let file_menu = Submenu::with_items(
                ...
                &[
                    ...
                    // Opens the in-app import wizard (format + destination picker).
                    &MenuItem::with_id(
                        app,
                        "import-screenplay",
                        "Import Screenplay...",
            ...
            let menu = Menu::with_items(
                app,
                &[&file_menu, &edit_menu, &format_menu, &view_menu, &help_menu],
            )?;
            app.set_menu(menu)?;
            ...
            app.on_menu_event(move |app, event| {
                ...
                match event.id().as_ref() {
                    "new-film" => {
                        let _ = app.emit("menu-new-film", ());
                    }
                    "new-series" => {
                        let _ = app.emit("menu-new-series", ());
                    }
                    "open" => {
                        let _ = app.emit("menu-open", ());
                    }
                    "import-screenplay" => {
                        let _ = app.emit("menu-import-screenplay", ());
                    }
                    "save" => {
                    ...
                    "help-guide" => {
                        let _ = app.emit("menu-help-guide", ());
                    }
                    "check-updates" => {
                    ...
                    "scene-cards" => {
                        let _ = app.emit("menu-scene-cards", ());
                    }
                    "story-mode" => {
                        let _ = app.emit("menu-story-mode", ());
                    }
                    "view-writing" => {
                        let _ = app.emit("menu-view-writing", ());
                    }
                    "toggle-sidebar" => {
                        let _ = app.emit("menu-toggle-sidebar", ());
                    }
                    "edit-meta" => {
                        let _ = app.emit("menu-edit-meta", ());
                    }
                    "export" => {
                    ...
                    "bold" => {
                        let _ = app.emit("menu-bold", ());
                    }
                    "italic" => {
                        let _ = app.emit("menu-italic", ());
                    }
                    "underline" => {
                        let _ = app.emit("menu-underline", ());
                    }
                    ...
                    "elem-scene-heading" => {
                        let _ = app.emit("menu-element-type", "scene_heading");
                    }
                    "elem-action" => {
                        let _ = app.emit("menu-element-type", "action");
                    }
                    "elem-character" => {
                        let _ = app.emit("menu-element-type", "character");
                    }
                    "elem-parenthetical" => {
                        let _ = app.emit("menu-element-type", "parenthetical");
                    }
                    "elem-dialogue" => {
                        let _ = app.emit("menu-element-type", "dialogue");
                    }
                    "elem-transition" => {
                        let _ = app.emit("menu-element-type", "transition");
                    }
                    // Theme submenu (#168)
                    "theme-light" => {
                    ...
                    "find-replace" => {
                        let _ = app.emit("menu-find-replace", ());
                    }
                    "quit" => {
                    ...
                    "report-issue" => {
                        ...
                        let _ = app
                            .opener()
                            .open_url("https://github.com/stultus/scriptty/issues", None::<&str>);
                    }
                    "view-github" => {
                        use tauri_plugin_opener::OpenerExt;
                        let _ = app
                            .opener()
                            .open_url("https://github.com/stultus/scriptty", None::<&str>);
                    }
