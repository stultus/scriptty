			...
			>
				<header class="ep-header">
					<span
						...
						title="Drag to reorder">{String(ep.number).padStart(2, '0')}</span
					>
					...
					{:else}
						<button
							...
							title="Click to rename">{ep.title.trim() || 'Untitled'}</button
						>
					{/if}
				...
				<footer class="ep-footer">
					<div class="ep-stats">
						<button
							...
							onclick={(e) => {
								e.stopPropagation();
								cycleStatus(index, ep.status ?? 'outline');
							}}
...
<style>
	...
	.ep-status {
		...
		cursor: pointer;
		transition:
			filter var(--motion-fast, 100ms) ease,
			transform var(--motion-fast, 100ms) ease;
	}
