<script lang="ts">
	...
	let { open = $bindable(false), onEditMetadata = () => {} } = $props<{
		open: boolean;
	...
	function buildSeriesExportDocument(): ScreenplayDocument | null {
		...
		for (let i = 0; i < episodes.length; i++) {
			...
			const label = epTitle ? `EPISODE ${ep.number}: ${epTitle}` : `EPISODE ${ep.number}`;
			combinedBlocks.push({
	...
	function buildDocumentForExport(): ScreenplayDocument | null {
		...
		const base =
			isSeriesProject && exportScope === 'series'
	...
	function buildEpisodeFilmDocument(episodeIndex: number): ScreenplayDocument | null {
		...
		const composedTitle = epTitle
			? seriesTitle
				? `${seriesTitle} — Ep ${ep.number}: ${epTitle}`
				: `Ep ${ep.number}: ${epTitle}`
			: seriesTitle
				? `${seriesTitle} — Ep ${ep.number}`
				: `Episode ${ep.number}`;
		return {
			type: 'film',
			series: null,
		...
		<div class="modal-card export-card" use:focusTrap>
			...
			<header class="export-header">
				...
				<button
					...
					aria-label="Close export">&times;</button
				>
			</header>
			...
			{#if isSeriesProject}
				<div class="series-strip">
					<div class="series-strip-meta">
						...
						<span class="series-strip-value"
							>{documentStore.document?.series?.title || 'Untitled Series'}</span
						>
						<span class="series-strip-ep"
							>Episode {documentStore.activeEpisode?.number ?? ''}{documentStore.activeEpisode
								?.title
								? ` — ${documentStore.activeEpisode.title}`
								: ''}</span
						>
					</div>
				...
				<section class="col-sections" aria-labelledby="sections-heading">
					<div class="col-heading">
						...
						<div class="col-heading-actions">
							<button
								...
								disabled={anyExporting}
								title="Turn on every available section"
							>
								All
							</button>
                 ...
                 toggle. Disabled tiles dim down (e.g. Synopsis when empty). -->
						...
						<button
							...
							onclick={() => {
								includeScreenplay = !includeScreenplay;
							}}
						...
						>
							...
							<span class="tile-body">
								...
								<span class="tile-hint"
									>{sceneCount} {sceneCount === 1 ? 'scene' : 'scenes'} · main content</span
								>
							</span>
						...
						>
							...
							<span class="tile-body">
								...
								<span class="tile-hint"
									>{hasSynopsis ? 'Story panel — short prose' : 'Add text in the Story panel'}</span
								>
							</span>
							<span class="tile-check"></span>
						</button>
						...
						>
							...
							<span class="tile-body">
								...
								<span class="tile-hint"
									>{hasTreatment ? 'Scene-by-scene prose' : 'Add text in the Story panel'}</span
								>
							</span>
							<span class="tile-check"></span>
						</button>
						...
						>
							...
							<span class="tile-body">
								...
								<span class="tile-hint"
									>{hasNarrative ? 'Long-form story' : 'Add text in the Story panel'}</span
								>
							</span>
							<span class="tile-check"></span>
						</button>
						...
						<button
							...
							onclick={() => {
								includeSceneCards = !includeSceneCards;
							}}
						...
						{#if hasScheduledScenes}
							<button
								...
								onclick={() => {
									includeShootList = !includeShootList;
								}}
				...
				<section class="col-options" aria-labelledby="format-heading">
					...
					{:else}
						<div class="format-empty">
							<p>
								Format applies to the screenplay section. Turn that on to choose Hollywood or
								Indian.
							</p>
						</div>
					{/if}
						...
						{#if includeScreenplay}
							<label class="opt-row">
								...
								<button
									...
									onclick={() => {
										pageBreakAfterScene = !pageBreakAfterScene;
							...
							<label class="opt-row">
								...
								<button
									...
									onclick={() => {
										charactersBelowHeading = !charactersBelowHeading;
									}}><span class="opt-thumb"></span></button
								>
							</label>
						{/if}
						...
						{#if includeSceneCards}
							<label class="opt-row">
								...
								<button
									...
									onclick={() => {
										compactSceneCards = !compactSceneCards;
									}}><span class="opt-thumb"></span></button
								>
							</label>
						{/if}
						...
						{#if sceneCount > 0 && (includeScreenplay || includeSceneCards) && !(isSeriesProject && exportScope === 'series')}
							<label class="opt-row">
								...
								<button
									...
									onclick={() => {
										sceneRangeEnabled = !sceneRangeEnabled;
									}}><span class="opt-thumb"></span></button
								>
							</label>
			...
			<footer class="export-footer">
				<button
					...
					disabled={anyExporting}>Cancel</button
				>
				...
				<button class="btn-secondary" onclick={handlePlaintextExport} disabled={anyExporting}>
					...
					{#if exportingPlaintext}<span class="spinner" aria-hidden="true"
						></span>Exporting{:else}Plain text{/if}
				</button>
				<button class="btn-secondary" onclick={handleFountainExport} disabled={anyExporting}>
					...
					{#if exportingFountain}<span class="spinner" aria-hidden="true"
						></span>Exporting{:else}Fountain{/if}
				</button>
				<button class="btn-primary" onclick={handleExport} disabled={anyExporting}>
					...
					{#if exporting}<span class="spinner spinner-on-primary" aria-hidden="true"
						></span>Generating PDF{:else}Export PDF{/if}
				</button>
			</footer>
		</div>
	</div>
