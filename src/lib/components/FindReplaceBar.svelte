...
<div class="find-replace-bar">
	<div class="find-row">
		...
		<span
			class="match-info"
			class:hint={query.length === 0}
			class:empty={query.length > 0 && matchCount === 0}
		>
			{#if matchCount > 0}
		...
		<button class="bar-btn" class:active={caseSensitive} onclick={toggleCase} title="Match case"
			>Aa</button
		>
	...
	{#if mode === 'replace'}
		<div class="replace-row">
			...
			/>
			<button
				class="bar-btn replace-action"
				onclick={handleReplace}
				disabled={matchCount === 0}
				title="Replace current match (Enter)">Replace</button
			>
			<button
				class="bar-btn replace-action"
				onclick={handleReplaceAll}
				disabled={matchCount === 0}
				title="Replace every match">Replace All</button
			>
		</div>
	{/if}
</div>
