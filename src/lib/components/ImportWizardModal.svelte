<script lang="ts">
  ...
  const seriesTitle = $derived(
    documentStore.document?.series?.title?.trim() || 'Untitled Series',
  );
  ...
  >
    <div class="modal-card import-wizard-card" use:focusTrap>
      <header class="wizard-header">
        ...
        <button class="btn-close" onclick={handleCancel} aria-label="Close import wizard">&times;</button>
      </header>
        ...
        <div class="wizard-section" role="group" aria-labelledby="wizard-format-label">
          ...
          <div class="card-grid">
            <button
              ...
              onclick={() => { format = 'fountain'; }}
            >
              <span class="choice-name">Fountain</span>
              <span class="choice-sub">Plain-text screenplay format. Highland, Slugline, WriterDuet, Beat.</span>
            </button>
            <button
              type="button"
              class="choice-card"
            ...
            >
              ...
              <span class="choice-sub">XML format from Final Draft 8 onward — <code>.fdx</code> files.</span>
            </button>
          </div>
        </div>
        ...
        <div class="wizard-section" role="group" aria-labelledby="wizard-destination-label">
          ...
          <div class="card-grid">
            ...
            <button
              ...
              disabled={!isSeries}
              onclick={() => { if (isSeries) destination = 'episode'; }}
            >
              <span class="choice-name">
                As an episode{#if isSeries} of <em class="series-em">{seriesTitle}</em>{/if}
              </span>
