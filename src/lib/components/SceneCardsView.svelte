<script lang="ts">
	...
	let heroCountValue = $derived(
		heroIsEpisodesLevel ? (documentStore.document?.series?.episodes?.length ?? 0) : cards.length
	);
	...
	function reorderScene(fromNumber: number, toNumber: number) {
		...
		const fromChildEnd =
			fromIdx + 1 < sceneBounds.length ? sceneBounds[fromIdx + 1].childIndex : doc.childCount;
		...
		const fromEndPos =
			fromIdx + 1 < sceneBounds.length ? sceneBounds[fromIdx + 1].offset : doc.content.size;
		...
		if (toNumber < fromNumber) {
			...
			insertPos = toIdx + 1 < sceneBounds.length ? sceneBounds[toIdx + 1].offset : doc.content.size;
		}
	...
	async function deleteScene(sceneNumber: number, sceneOrder: number) {
		...
		const endPos =
			sceneOrder + 1 < sceneBounds.length ? sceneBounds[sceneOrder + 1].offset : doc.content.size;
		...
		<div class="hero-left">
				...
				{#if heroIsSeriesScenes}
					...
					>
						<svg
							...
							aria-hidden="true"><path d="M15 18 L9 12 L15 6" /></svg
						>
						<span>Episodes</span>
					</button>
					<span class="eyebrow-sep" aria-hidden="true">·</span>
					<span class="eyebrow-context"
						>Episode {String(documentStore.activeEpisode?.number ?? 1).padStart(2, '0')}</span
					>
			...
			<p class="hero-subtitle">
				{#if heroIsEpisodesLevel}
					...
					{#if seriesTotals}
						<span class="subtitle-meta">
							...
							<strong>{seriesTotals.scenes}</strong>
							{seriesTotals.scenes === 1 ? 'scene' : 'scenes'}
							<span class="meta-sep" aria-hidden="true">·</span>
				...
				{:else if heroIsSeriesScenes}
					<span class="subtitle-name"
						>{documentStore.activeEpisode?.title?.trim() || 'Untitled episode'}</span
					>
				{:else}
					<span class="subtitle-name"
						>{documentStore.activeMeta?.title?.trim() || 'Untitled screenplay'}</span
					>
				{/if}
			</p>
		</div>
	...
	{:else}
		<div class="cards-grid" class:compact={sceneCompact} bind:this={gridEl}>
			{#each displayCards as card (card.key)}
				...
				{@const cardSetting =
					headingUpper.startsWith('INT./EXT.') || headingUpper.startsWith('INT/EXT')
						? 'INT_EXT'
             ...
             the footer hairline. -->
					<div
						...
						onmousedown={(e: MouseEvent) => {
							if (!groupByLocation) startDrag(e, card.sceneNumber);
						}}
						role="button"
						tabindex="-1"
						...
						title={groupByLocation
							? 'Switch off "Group by location" to drag-reorder'
							: 'Drag to reorder'}
					>
                 ...
                 ornaments at narrow column widths. -->
						...
						{:else}
							<button
								...
								title="Click to rename">{headingUpper || 'Untitled scene'}</button
							>
						{/if}

						{#if card.characters.length > 0}
							<p class="card-cast">
								<span class="cast-mark" aria-hidden="true">w/</span>
						...
						<div class="card-editable">
							<textarea
								...
								oninput={(e) =>
									updateDescription(card.sceneOrder, (e.target as HTMLTextAreaElement).value)}
                 ...
                 hint of the populated values. -->
							<details class="production-disclosure" open={hasProduction}>
								<summary class="production-summary">
									...
									{:else}
										<span class="production-hint muted"
											>cast extras · location group · shoot date</span
										>
									{/if}
								</summary>
								<div class="production-body">
									...
									<div class="prod-row">
										<div class="prod-field">
											<label class="prod-label" for="locgroup-{card.sceneNumber}"
							...
							<textarea
								...
								oninput={(e) =>
									updateShootNotes(card.sceneOrder, (e.target as HTMLTextAreaElement).value)}
								onkeydown={handleKeydown}
							></textarea>
						</div>
