<script lang="ts">
	...
	let isActiveEpisode = $derived(
		episodeIndex === undefined || episodeIndex === documentStore.activeEpisodeIndex
	);
	...
	function doScrollToScene(sceneIndex: number) {
		...
		requestAnimationFrame(() => {
			...
			const scrollContainer =
				view.dom.closest('.editor-scroll') ?? view.dom.parentElement?.parentElement;
			...
			{#each scenes as scene, sceneArrayIdx (scene.index)}
				{@const sceneOrder =
					...
					(episodeIndex !== undefined
						? (documentStore.document?.series?.episodes?.[episodeIndex]?.settings
				...
				<li
					...
					class:drop-above={dropTargetScene === scene.number &&
						dragFromScene !== null &&
						dragFromScene > scene.number}
					class:drop-below={dropTargetScene === scene.number &&
						dragFromScene !== null &&
						dragFromScene < scene.number}
				...
				>
                 ...
                 the left edge already signals setting via color). -->
						...
						<span class="scene-text"
							>{stripSettingPrefix(scene.text).toUpperCase() || '(empty)'}</span
						>
