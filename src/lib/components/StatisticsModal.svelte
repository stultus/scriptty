<script lang="ts">
	...
	let activeTabLabel = $derived.by<{ eyebrow: string; title: string }>(() => {
		switch (activeTab) {
			case 'overview':
				return { eyebrow: 'At a glance', title: 'The <em>script</em> at a glance' };
			case 'characters':
				return { eyebrow: 'Cast report', title: 'Characters by <em>dialogue</em>' };
			case 'locations':
				return { eyebrow: 'Production prep', title: 'Where it <em>happens</em>' };
			case 'schedule':
				return { eyebrow: 'Production prep', title: 'The <em>shoot</em> schedule' };
			case 'episodes':
				return { eyebrow: 'Series report', title: 'Every <em>episode</em>, side by side' };
		}
	});
	...
	async function exportLocationsCsv() {
		const rows: Array<Array<string | number>> = [['Rank', 'Location', 'Scenes', 'Setting']];
	...
	async function exportScheduleCsv() {
		...
		const source = schedSortKey === null ? groupedSchedule.flatMap((g) => g.rows) : sortedSchedule;
	...
	function computeStatsFor(
		...
		for (const node of nodes) {
			...
			if (type === 'scene_heading') {
				...
				sceneCharCount.set(currentScene, (sceneCharCount.get(currentScene) ?? 0) + text.length);
			}
               ...
               the narrower rail without losing meaning. -->
					<div class="scope-segmented" role="group" aria-label="Statistics scope">
						<button
							...
							title="Stats for the active episode">Episode</button
						>
						<button
							type="button"
							class="scope-seg"
							...
							title="Stats aggregated across every episode">Series</button
						>
					</div>
				{/if}
				...
				<header class="pane-header">
					...
					<div class="pane-actions">
						...
						<button
							...
							aria-label="Close statistics">&times;</button
						>
					</div>
				</header>
						...
						<div class="hero-grid">
							...
							<div class="hero-stat">
								<span class="hero-value"
									>~{stats.screenTimeMinutes}<span class="hero-unit">min</span></span
								>
								...
								<div class="bar-pair">
									<div class="bar-row">
										...
										<span class="bar-track"
											><span
												class="bar-fill"
												style="width: {(stats.intCount / totalSetting) * 100}%"
											></span></span
										>
									...
									<div class="bar-row">
										...
										<span class="bar-track"
											><span
												class="bar-fill alt"
												style="width: {(stats.extCount / totalSetting) * 100}%"
											></span></span
										>
								...
								<div class="bar-pair">
									<div class="bar-row">
										...
										<span class="bar-track"
											><span
												class="bar-fill warm"
												style="width: {(stats.dayCount / totalTime) * 100}%"
											></span></span
										>
									...
									<div class="bar-row">
										...
										<span class="bar-track"
											><span
												class="bar-fill cool"
												style="width: {(stats.nightCount / totalTime) * 100}%"
											></span></span
										>
								...
								<table class="data-table">
									<thead>
										<tr>
											<th
												class="col-name"
												aria-sort={ariaSortFor(charSortKey === 'name', charSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleCharSort('name')}>
													Character
													<span
														class="sort-ind"
														class:active={charSortKey === 'name'}
														data-dir={charSortKey === 'name' ? charSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(charSortKey === 'scenes', charSortDir)}
											>
												<button class="sort-btn align-end" onclick={() => cycleCharSort('scenes')}>
													Scenes
													<span
														class="sort-ind"
														class:active={charSortKey === 'scenes'}
														data-dir={charSortKey === 'scenes' ? charSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(charSortKey === 'dialogueBlocks', charSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleCharSort('dialogueBlocks')}
												>
													Dialogue
													<span
														class="sort-ind"
														class:active={charSortKey === 'dialogueBlocks'}
														data-dir={charSortKey === 'dialogueBlocks' ? charSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-bar"
												aria-sort={ariaSortFor(charSortKey === 'percentage', charSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleCharSort('percentage')}>
													Share
													<span
														class="sort-ind"
														class:active={charSortKey === 'percentage'}
														data-dir={charSortKey === 'percentage' ? charSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											...
											<tr>
												...
												<td class="col-bar">
													<span class="inline-bar"
														><span class="inline-bar-fill" style="width: {char.percentage}%"
														></span></span
													>
					...
					{:else if activeTab === 'locations'}
								...
								<table class="data-table">
									<thead>
										<tr>
											<th
												class="col-name"
												aria-sort={ariaSortFor(locSortKey === 'name', locSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleLocSort('name')}>
													Location
													<span
														class="sort-ind"
														class:active={locSortKey === 'name'}
														data-dir={locSortKey === 'name' ? locSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(locSortKey === 'scenes', locSortDir)}
											>
												<button class="sort-btn align-end" onclick={() => cycleLocSort('scenes')}>
													Scenes
													<span
														class="sort-ind"
														class:active={locSortKey === 'scenes'}
														data-dir={locSortKey === 'scenes' ? locSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(locSortKey === 'setting', locSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleLocSort('setting')}>
													Setting
													<span
														class="sort-ind"
														class:active={locSortKey === 'setting'}
														data-dir={locSortKey === 'setting' ? locSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
									...
									<tbody>
										{#each sortedLocations as loc, i}
											<tr>
												...
												<td class="col-meta"
													><span class="setting-pill setting-{loc.setting.toLowerCase()}"
														>{loc.setting}</span
													></td
												>
											</tr>
						...
						{:else}
							<div class="empty-pane">
								<span class="empty-glyph">∅</span>
								<p>
									No locations parsed — write scene headings like <em>INT. KITCHEN — DAY</em> to populate
									this report.
								</p>
							</div>
						{/if}
								...
								<table class="data-table schedule-table">
									<thead>
										<tr>
											<th
												class="col-snum"
												aria-sort={ariaSortFor(schedSortKey === 'sceneNumber', schedSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleSchedSort('sceneNumber')}
												>
													#
													<span
														class="sort-ind"
														class:active={schedSortKey === 'sceneNumber'}
														data-dir={schedSortKey === 'sceneNumber' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(schedSortKey === 'setting', schedSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleSchedSort('setting')}>
													Setting
													<span
														class="sort-ind"
														class:active={schedSortKey === 'setting'}
														data-dir={schedSortKey === 'setting' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-name"
												aria-sort={ariaSortFor(schedSortKey === 'location', schedSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleSchedSort('location')}>
													Location
													<span
														class="sort-ind"
														class:active={schedSortKey === 'location'}
														data-dir={schedSortKey === 'location' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(schedSortKey === 'time', schedSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleSchedSort('time')}>
													Time
													<span
														class="sort-ind"
														class:active={schedSortKey === 'time'}
														data-dir={schedSortKey === 'time' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(schedSortKey === 'characterCount', schedSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleSchedSort('characterCount')}
												>
													Cast
													<span
														class="sort-ind"
														class:active={schedSortKey === 'characterCount'}
														data-dir={schedSortKey === 'characterCount' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(schedSortKey === 'locationGroup', schedSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleSchedSort('locationGroup')}>
													Group
													<span
														class="sort-ind"
														class:active={schedSortKey === 'locationGroup'}
														data-dir={schedSortKey === 'locationGroup' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(schedSortKey === 'scheduledDate', schedSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleSchedSort('scheduledDate')}>
													Day
													<span
														class="sort-ind"
														class:active={schedSortKey === 'scheduledDate'}
														data-dir={schedSortKey === 'scheduledDate' ? schedSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
												...
												<tr class="schedule-group-header" class:unscheduled={group.date === ''}>
													<td colspan="7">
														...
														<span class="sg-meta">
															<span class="sg-count"
																>{group.rows.length}
																{group.rows.length === 1 ? 'scene' : 'scenes'}</span
															>
								...
								<table class="data-table episodes-table">
									<thead>
										<tr>
											<th
												class="col-snum"
												aria-sort={ariaSortFor(epSortKey === 'number', epSortDir)}
											>
												<button class="sort-btn align-end" onclick={() => cycleEpSort('number')}>
													#
													<span
														class="sort-ind"
														class:active={epSortKey === 'number'}
														data-dir={epSortKey === 'number' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-name"
												aria-sort={ariaSortFor(epSortKey === 'title', epSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleEpSort('title')}>
													Title
													<span
														class="sort-ind"
														class:active={epSortKey === 'title'}
														data-dir={epSortKey === 'title' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-meta"
												aria-sort={ariaSortFor(epSortKey === 'status', epSortDir)}
											>
												<button class="sort-btn" onclick={() => cycleEpSort('status')}>
													Status
													<span
														class="sort-ind"
														class:active={epSortKey === 'status'}
														data-dir={epSortKey === 'status' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'pageCount', epSortDir)}
											>
												<button class="sort-btn align-end" onclick={() => cycleEpSort('pageCount')}>
													Pages
													<span
														class="sort-ind"
														class:active={epSortKey === 'pageCount'}
														data-dir={epSortKey === 'pageCount' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'sceneCount', epSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleEpSort('sceneCount')}
												>
													Scenes
													<span
														class="sort-ind"
														class:active={epSortKey === 'sceneCount'}
														data-dir={epSortKey === 'sceneCount' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'wordCount', epSortDir)}
											>
												<button class="sort-btn align-end" onclick={() => cycleEpSort('wordCount')}>
													Words
													<span
														class="sort-ind"
														class:active={epSortKey === 'wordCount'}
														data-dir={epSortKey === 'wordCount' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'dialogueLineCount', epSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleEpSort('dialogueLineCount')}
												>
													Dialogue
													<span
														class="sort-ind"
														class:active={epSortKey === 'dialogueLineCount'}
														data-dir={epSortKey === 'dialogueLineCount' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'screenTimeMinutes', epSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleEpSort('screenTimeMinutes')}
												>
													~min
													<span
														class="sort-ind"
														class:active={epSortKey === 'screenTimeMinutes'}
														data-dir={epSortKey === 'screenTimeMinutes' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
											<th
												class="col-num"
												aria-sort={ariaSortFor(epSortKey === 'characterCount', epSortDir)}
											>
												<button
													class="sort-btn align-end"
													onclick={() => cycleEpSort('characterCount')}
												>
													Cast
													<span
														class="sort-ind"
														class:active={epSortKey === 'characterCount'}
														data-dir={epSortKey === 'characterCount' ? epSortDir : ''}
														aria-hidden="true"
													></span>
												</button>
											</th>
									...
									<tbody>
										{#each sortedEpisodes as ep (ep.number)}
											<tr>
												<td class="col-snum"
													><span class="ep-pip">{String(ep.number).padStart(2, '0')}</span></td
												>
												<td class="col-name">
													{#if ep.title}{ep.title}{:else}<span class="ep-untitled">Untitled</span
														>{/if}
												</td>
												<td class="col-meta"
													><span class="status-pill status-{ep.status}">{ep.status}</span></td
												>
