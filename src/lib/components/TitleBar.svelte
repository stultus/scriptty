<script lang="ts">
	 ...
	 *  cover sheet. (#165) */
	let metadataIncomplete = $derived(!documentStore.activeMeta?.title?.trim());
	...
	$effect(() => {
		...
		if (documentStore.isDirty && recentlySaved) {
			recentlySaved = false;
			if (savedResetTimeout) {
				clearTimeout(savedResetTimeout);
				savedResetTimeout = null;
			}
		}
	});
...
<div class="title-bar">
	...
	<div class="title-zone">
		...
		{#if documentStore.isSeries && documentStore.activeEpisode}
			<button
				...
				type="button"
				onclick={() => {
					episodePopoverOpen = !episodePopoverOpen;
				}}
			...
			>
				<span class="ep-badge-mini"
					>{String(documentStore.activeEpisode.number).padStart(2, '0')}</span
				>
	...
	<div class="btn-group right">
		...
		<button class="btn-ghost" onclick={() => onShowExport?.()} title="Export document"
			>Export</button
		>
		<button
			class="btn-primary"
