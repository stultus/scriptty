...
class DocumentStore {
	...
	reorderEpisode(from: number, to: number): void {
		...
		else if (from < this.activeEpisodeIndex && to >= this.activeEpisodeIndex)
	...
	async openDocument(path: string): Promise<void> {
		try {
			...
			const loaded =
				doc.type === 'series' ? (doc.series?.episodes?.[0]?.content ?? doc.content) : doc.content;
	...
	async saveAsDialog(): Promise<void> {
		...
		const path = await save({
			defaultPath: defaultTitle ? `${defaultTitle}.screenplay` : 'untitled.screenplay',
			filters: [{ name: 'Screenplay', extensions: ['screenplay'] }]
		});
		if (!path) return; // User cancelled
		await this.saveDocument(path);
