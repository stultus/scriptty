<script lang="ts">
	...
	let showAnnotations = $state(
		typeof localStorage !== 'undefined'
			? localStorage.getItem('scriptty-annotations') !== 'false'
			: true
	);
	let showOutlinePeek = $state(
		typeof localStorage !== 'undefined'
			? localStorage.getItem('scriptty-outline-peek') === 'true'
			: false
	);
	...
	function pushRecentFile(path: string) {
		...
		try {
			const name = (path.split('/').pop() ?? path.split('\\').pop() ?? path).replace(
				/\.screenplay$/,
				''
			);
	...
	async function openFileDialog() {
		...
		const path = await open({
			...
			filters: [
				{
					name: 'Screenplay, Fountain, or Final Draft',
					extensions: ['screenplay', 'fountain', 'fdx']
				},
		...
		if (lower.endsWith('.fountain')) {
			...
			if (summary)
				importToast = { summary: { kind: 'fountain', ...summary }, filename: basename(path) };
	...
	let commands = $derived<Command[]>([
		...
		},
		{
			...
			label: 'Save',
			hint: '⌘S',
			action: () => documentStore.saveWithDialog()
		},
		{
			id: 'file.saveas',
			group: 'File',
			label: 'Save As…',
			hint: '⌘⇧S',
			action: () => documentStore.saveAsDialog()
		},
		{
			id: 'file.export',
			group: 'File',
			label: 'Export…',
			keywords: 'pdf fountain plain text hollywood indian',
			action: () => {
				showExport = true;
			}
		},
		{
			...
			group: 'File',
			label: 'Edit Metadata…',
			keywords: 'title author director contact draft',
			action: () => {
				showMetadata = true;
			}
		},
		...
		},
		{
			...
			hint: '⌘\\',
			keywords: 'sidebar panel scenes episodes',
			action: togglePanel
		},
		{
			id: 'view.outline',
			group: 'View',
			label: 'Toggle Outline Peek',
			hint: '⌘⇧O',
			keywords: 'timeline strip',
			action: toggleOutlinePeek
		},
		{
			id: 'view.annotations',
			group: 'View',
			label: 'Toggle Scene Annotations',
			hint: '⌘⇧A',
			keywords: 'notes comments',
		...
		// Settings
		{
			...
			group: 'Settings',
			label: 'Settings…',
			keywords: 'language font scheme theme preferences',
			action: () => {
				showSettings = true;
			}
		},
		{
			id: 'settings.theme',
			group: 'Settings',
			label: `Switch to ${themeStore.isDark ? 'Light' : 'Dark'} Mode`,
			keywords: 'appearance',
			action: () => {
				themeStore.toggle();
			}
		},
		{
			id: 'settings.lang',
			group: 'Settings',
			label: `Switch to ${isMalayalam ? 'English' : 'Malayalam'} Input`,
			hint: '⌃Space',
			keywords: 'mal eng mode',
			action: () => {
				inputManager.toggle();
			}
		},
		{
			id: 'settings.scheme.mozhi',
			group: 'Settings',
			label:
				inputScheme === 'mozhi'
					? 'Malayalam Scheme: Mozhi (active)'
					: 'Use Malayalam Scheme: Mozhi',
			keywords: 'keyboard transliteration phonetic',
			action: () => {
				inputManager.setScheme('mozhi');
				if (!isMalayalam) inputManager.toggle();
			}
		},
		{
			id: 'settings.scheme.inscript2',
			group: 'Settings',
			label:
				inputScheme === 'inscript2'
					? 'Malayalam Scheme: Inscript 2 (active)'
					: 'Use Malayalam Scheme: Inscript 2',
			keywords: 'keyboard layout',
			action: () => {
				inputManager.setScheme('inscript2');
				if (!isMalayalam) inputManager.toggle();
			}
		},
		{
			id: 'settings.scheme.inscript1',
			group: 'Settings',
			label:
				inputScheme === 'inscript1'
					? 'Malayalam Scheme: Inscript 1 (active)'
					: 'Use Malayalam Scheme: Inscript 1',
			keywords: 'keyboard layout legacy',
			action: () => {
				inputManager.setScheme('inscript1');
				if (!isMalayalam) inputManager.toggle();
			}
		},
		...
		// Help
		{
			id: 'help.guide',
			group: 'Help',
			label: 'How to Use Scriptty',
			keywords: 'help guide shortcuts',
			action: () => {
				showHelp = true;
			}
		},
	...
	onMount(() => {
		...
		(async () => {
			track(
				await listen('menu-new-film', async () => {
					if (!(await documentStore.confirmIfDirty())) return;
					showFilmDialog = true;
				})
			);
			...
			track(
				await listen('menu-new-series', async () => {
					if (!(await documentStore.confirmIfDirty())) return;
					showSeriesDialog = true;
				})
			);
			...
			track(
				await listen('menu-import-screenplay', () => {
					showImportWizard = true;
				})
			);
			...
			track(
				await listen('menu-scene-cards', () => {
					activeView = activeView === 'cards' ? 'writing' : 'cards';
				})
			);

			track(
				await listen('menu-story-mode', () => {
					activeView = activeView === 'story' ? 'writing' : 'story';
				})
			);
			...
			track(
				await listen('menu-close-window', async () => {
					if (!(await documentStore.confirmIfDirty())) return;
					quitConfirmed = true;
			...
			// Cards / Story.
			track(
				await listen('menu-view-writing', () => {
					activeView = 'writing';
				})
			);
			...
			track(
				await listen<string>('menu-element-type', (event) => {
			...
			track(
				await listen<{ requested: string; fallback: string }>('font-fallback', (event) => {
		...
		<div class="workspace">
         ...
         is position:absolute and anchors to .workspace's relative box. -->
			...
			<div class="editor-area" class:hidden={activeView !== 'writing'}>
				<Editor
					bind:findReplaceOpen
					bind:findReplaceMode
					{showAnnotations}
...
<NewProjectDialog
	bind:open={showSeriesDialog}
	kind="series"
	onConfirm={handleCreateSeriesFromDialog}
/>
<ImportWizardModal
